...
class TestSync(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with sqlite3.connect(self.DB) as conn:
            conn.execute("""CREATE TABLE IF NOT EXISTS contacts (
                uuid TEXT PRIMARY KEY, name TEXT, age TEXT, phone TEXT
            )""")
    ...
    def test_export_logic(self):
        ...
        with open(self.CSV, 'r') as f:
            ...
            self.assertEqual(reader[0], ['uuid', 'name', 'age', 'phone'])
            self.assertEqual(reader[1], ['g1', 'Alice', '30', '555'])

    def test_upsert_logic(self):
        """Verify 'uuid' allows both updating existing and inserting new rows."""
        data = {
            'uuid':'g1',
            'name':'Old',
            'age':'20',
        ...
        with open(self.CSV, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['uuid', 'name', 'age', 'phone'])
        ...
        with sqlite3.connect(self.DB) as conn:
            upd = conn.execute("SELECT name FROM contacts WHERE uuid='g1'").fetchone()[0]
            ins = conn.execute("SELECT name FROM contacts WHERE uuid='g2'").fetchone()[0]
    ...
    def test_schema_agnostic(self):
        ...
        with sqlite3.connect(self.DB) as conn:
            conn.execute("CREATE TABLE IF NOT EXISTS products (uuid TEXT PRIMARY KEY, price REAL)")
